...
class FreeConvNetwork(nn.Module):
    def __init__(self):
        ...
        self.freeConvStack = nn.Sequential( #define the order in which data passes through net
            LocallyConnected2d(in_channels=3, out_channels=8, output_size=(107,87), kernel_size=6, stride=2, bias=True),
            ...
            LocallyConnected2d(16, 32, (23,18), 6, 2, True),
            nn.ReLU(),

            nn.MaxPool2d(kernel_size=2),
            
            nn.Flatten(),
            nn.Linear(32*11*9, 300), # input size = output_channels*(output_size/2) of last Local layer because of maxpool2d kernel size 2
            nn.ReLU(),
            nn.Linear(300, 200)
        )
