...
class FreeConvNetwork(nn.Module):
    def __init__(self):
        ...
        self.freeConvStack = nn.Sequential( #define the order in which data passes through net
            ...
            LocallyConnected2d(1, 1, (87, 67), 6, 1, True),
            nn.ReLU(),

            LocallyConnected2d(1, 1, (82, 62), 6, 1, True),
            nn.ReLU(),

            nn.MaxPool2d(kernel_size=2),
            
            nn.Flatten(),
            nn.Linear(1*41*31, 300), # input size = output_channels*(output_size/2) of last Local layer because of maxpool2d kernel size 2
            nn.ReLU(),
            nn.Linear(300, 200)
        )
